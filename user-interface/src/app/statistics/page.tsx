...
export default function StatisticsPage() {
  ...
  const hoveredShot = aerialShots.find((s) => s.id === hoveredId);

  return (
    <div className="mosaic-bg min-h-screen px-4 py-12">
      <div className="mx-auto max-w-4xl">
        <h1 className="text-3xl font-bold text-zinc-800">Statistics</h1>
        <p className="mt-2 text-zinc-600">Aerial view — shot landing locations</p>

        {/* Aerial table + shot overlay */}
        <div className="mt-10 flex justify-center">
          ...
          >
            ...
            {/* Shot dots overlay (absolute within table area) */}
            <div
              className="absolute left-8 top-8"
              style={{ width: TABLE_WIDTH, height: TABLE_HEIGHT }}
            >
              {aerialShots.map((shot) => (
                <div
                  key={shot.id}
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-zinc-600">Point won</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="h-3 w-3 rounded-full bg-red-500 ring-2 ring-white/80" />
            <span className="text-sm text-zinc-600">Point lost</span>
          </div>
        </div>
        <p className="mt-2 text-center text-sm text-zinc-500">Hover over a dot for shot details</p>
      </div>
    </div>
  );
}
